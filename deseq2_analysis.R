...
#pdf("./Rplots/normalization_plots.pdf")
#for (i in seq(1, length(norm_plots), 1)) {
#	grid.arrange(grobs=norm_plots[i],ncol=1,nrow=1)
#}
#dev.off()

#pdf('./Rplots/pca_plots.pdf')
#pca_plots <- generate_pca_plots(rld_blind)
...
d <- data.frame(PC1=pca$x[,1],PC2=pca$x[,2],PC3=pca$x[,3],PC4=pca$x[,4],PC5=pca$x[,5],PC6=pca$x[,6],mito=mito,Ycsome=Ycsome)
percentVar <- round(100*attr(d,"percentVar"))

# Visualize Data in PCA After Regressing Out Variables
#pdf('./Rplots/sva_PCA_regressed.pdf')
#ggplot(d, aes(x=PC1,y=PC2, color=mito,shape=Ycsome))+geom_point(size=3)+xlab(paste0("PC1: ",percentVar[1],"% Variance"))+ylab(paste0("PC2: ", percentVar[2],"% Variance"))+ggtitle('PCA: Mito-Y Surrogate Variables Regressed Out')+coord_fixed()+scale_color_manual(values=c("red4","red2","darkorange3",'darkorange','darkgoldenrod1','gold1'))
#dev.off()
...
# Extract significant GO cats
emito_go_plot.df <- data.frame(category=enriched_svamito_go.df[enriched_svamito_go.df$over_represented_padj < 0.05,]$category,term=enriched_svamito_go.df[enriched_svamito_go.df$over_represented_padj < 0.05,]$term,numDEInCat=enriched_svamito_go.df[enriched_svamito_go.df$over_represented_padj < 0.05,]$numDEInCat,numInCat=enriched_svamito_go.df[enriched_svamito_go.df$over_represented_padj < 0.05,]$numInCat,padj=enriched_svamito_go.df[enriched_svamito_go.df$over_represented_padj < 0.05,]$over_represented_padj)
...
FlyAtlas_FPKM <- read.delim("./FlyAtlas/FlyAtlas_FPKM.txt", stringsAsFactors=FALSE)
...
#pdf("./Rplots/Y_testis_bp.pdf")
#ggplot(Y_testis_enrich,aes(x=reorder(name,enrich_val),y=enrich_val,fill=-log10(padj)))+geom_bar(position='dodge',stat='identity')+coord_flip()+scale_fill_gradient(low='dodgerblue4',high='deepskyblue2')+ylab("Testis Enrichment Value")+xlab("Gene Name") + labs(color="-log10(P-Adj)")
#dev.off()
...
# Visualize Results in Vertical Bar Plot
#pdf("./Rplots/Y_acp_bp.pdf")
#ggplot(Y_acp_enrich,aes(x=reorder(name,enrich_val),y=enrich_val,fill=-log10(padj)))+geom_bar(position='dodge',stat='identity')+coord_flip()+scale_fill_gradient(low='dodgerblue4',high='deepskyblue2')+ylab("Accessory Gland Enrichment Value")+xlab("Gene Name") + labs(color="-log10(P-Adj)")
#dev.off()
...
# Visualize Results in Vertical Bar Plot
#pdf("./Rplots/mito_testis_bp.pdf")
#ggplot(mito_testis_enrich,aes(x=reorder(name,enrich_val),y=enrich_val,fill=-log10(padj)))+geom_bar(position='dodge',stat='identity')+coord_flip()+scale_fill_gradient(low='darkorange3',high='darkorange')+ylab("Testis Enrichment Value")+xlab("Gene Name") + labs(color="-log10(P-Adj)")
#dev.off()
...
# Visualize Results in Vertical Bar Plot
#pdf("./Rplots/mito_acp_bp.pdf")
#ggplot(mito_acp_enrich,aes(x=reorder(name,enrich_val),y=enrich_val,fill=-log10(padj)))+geom_bar(position='dodge',stat='identity')+coord_flip()+scale_fill_gradient(low='darkorange3',high='darkorange')+ylab("Accessory Gland Enrichment Value")+xlab("Gene Name") + labs(color="-log10(P-Adj)")
#dev.off()
...
#pdf('./Rplots/final_counts.pdf',width=50,height=25)
...
#pdf('./Rplots/mitoY_heatmap.pdf',width=20, height=15)
#generate_heatmap(mitoY_genes,mitoY_gene_names,rld_blind,"mito")
#dev.off()
...
#pdf('./Rplots/metabolic_heatmap.pdf',width=20, height=10)
#generate_heatmap(metabolic_genes,metabolic_gene_names,rld_blind,"mito")
#dev.off()
...
#pdf('./Rplots/vision_heatmap.pdf',width=20, height=5)
#generate_heatmap(vis_genes,vis_gene_names,rld_blind,"Ycsome")
#dev.off()